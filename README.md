...
| Feature | Detail |
|---|---|
| **Essay generation** | GPT-5.5 via the Responses API with a file-backed default system prompt |
| **Cover image** | Generated by GPT Image; falls back to a styled Pillow image if unavailable |
...
| **Run logging** | Appends each user prompt and run metadata to `output/logs/essay_runs.jsonl` |
...
```
┌──────────────────────────────────────────────────────────────┐
│  1. GPT-5.5 generates a Markdown essay from your prompt      │
│  2. The prompt and run metadata are logged locally           │
│  3. GPT Image creates a thematic cover image                 │
...
* **Costs:** each run uses GPT-5.5 for text generation and, unless
  `--no-cover` is passed, GPT Image for cover creation. Check
